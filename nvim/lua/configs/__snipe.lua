	...
	require("snipe").setup({
		ui = {
			...
			buffer_format = {
				...
				function(snipe_buf_obj)
					local count = #vim.diagnostic.get(snipe_buf_obj.id, { severity = vim.diagnostic.severity.ERROR })
					return count > 0 and string.format("[%s]", utils.get_icon_by_name("error")) or "", "DiagnosticError"
				end,
				function(snipe_buf_obj)
					local count = #vim.diagnostic.get(snipe_buf_obj.id, { severity = vim.diagnostic.severity.WARN })
					return count > 0 and string.format("[%s]", utils.get_icon_by_name("warn")) or "", "DiagnosticWarn"
				end,
				function(snipe_buf_obj)
